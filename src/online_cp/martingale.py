...
from scipy.special import betaln, gammaln, logsumexp, gammainc
...
class BetaKernel(BettingStrategy):
    ...
    def __init__(self, bandwidth='beta-reference', window_size=None, normalize=True, min_sample_size=100, max_iter=20, bw_min=0.001, bw_max=0.5, growth_factor=1.1):
...
class GaussianKDE(BettingStrategy):
    ...
    def __init__(self, bandwidth='silverman', window_size=None, min_sample_size=100, max_iter=20, bw_min=0.001, bw_max=0.5, growth_factor=1.1):
        super().__init__(min_sample_size)
        self.bandwidth = bandwidth
        self.window_size = window_size
        self.max_iter = max_iter
        ...
        if check_integration:
             try:
                 ...
                 if not np.isclose(total_prob, 1.0, atol=1e-3):
                     warnings.warn(f"FixedStrategy PDF does not integrate to 1 (integral={total_prob:.4f}). This may invalidate the martingale.")
             except Exception as e:
                 warnings.warn(f"Could not verify FixedStrategy PDF integration: {e}")
...
class ExpertAggregationStrategy(BettingStrategy):
    ...
    Examples:
        ...
        >>> expert_good = FixedStrategy(pdf=lambda x: norm.pdf(x, 0.1, 0.1) / (norm.cdf(1,0.1,0.1)-norm.cdf(0,0.1,0.1)), check_integration=False)
    ...
    def update_betting_function(self, p_values):
        ...
        def next_agg_pdf(x): return np.dot(final_weights_for_next_step, [pdf(x) for pdf in current_pdfs])
        def next_agg_cdf(x): return np.dot(final_weights_for_next_step, [cdf(x) for cdf in current_cdfs])
    ...
    def plot_weights(self, expert_names=None, title="Evolution of Expert Weights", ax=None):
        ...
        if not self.expert_weights_history:
            warnings.warn("No weight history to plot.")
            return
...
class SimpleJumper(ConformalTestMartingale):
    ...
    def update_martingale_value(self, p):
        ...
        self.B_n_inv = lambda u: (epsilon_bar - 2) / (2 * epsilon_bar) + np.sqrt(epsilon_bar * (8 * u + epsilon_bar - 4) + 4) / (2 * epsilon_bar) if abs(epsilon_bar) > 1e-9 else u
...
class OnionMartingale(ConformalTestMartingale):
    ...
    def update_martingale_value(self, p):
        ...
        def _composite_b(u):
            val = 1.0
            x = u
            for i, (b, B) in enumerate(zip(current_b_ns, current_B_ns)):
                val *= b(x)
                x = B(x)
            return val
